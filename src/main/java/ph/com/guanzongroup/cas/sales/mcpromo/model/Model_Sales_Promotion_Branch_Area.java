...
public class Model_Sales_Promotion_Branch_Area extends Model {
    ...
    public Model_Branch_Area BranchArea() throws SQLException, GuanzonException {
        if (poBranchArea == null) {
            poBranchArea = new Model_Branch_Area();
            poBranchArea.setApplicationDriver(poGRider);
            poBranchArea.setXML("Model_Branch_Area");
            poBranchArea.setTableName("Branch_Area");
            poBranchArea.initialize();
        }

        String branchArea = getValue("sAreaCode") == null ? "" : (String) getValue("sAreaCode");

        if (!"".equals(branchArea)) {
            if (poBranchArea.getEditMode() == EditMode.READY
                    && poBranchArea.getAreaCode().equals(branchArea)) {
                return poBranchArea;
            } else {
                if (ReferenceCache.tryLoad("Branch_Area", branchArea, poBranchArea)) {
                    return poBranchArea;
                }

                poJSON = poBranchArea.openRecord(branchArea);

                if ("success".equals((String) poJSON.get("result"))) {
                    ReferenceCache.store("Branch_Area", branchArea, poBranchArea);
                    return poBranchArea;
                } else {
                    poBranchArea.initialize();
                    return poBranchArea;
